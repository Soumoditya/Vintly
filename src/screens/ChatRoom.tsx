...
import { ArrowLeft, Send, Image as ImageIcon, Mic, Square, Smile, X } from 'lucide-react'
import { useAuth } from '../lib/auth'
import { listenMessages, sendMessage, uploadMedia, type Message } from '../lib/chat'
  ...
  return (
    <div className="flex h-screen flex-col" style={wallpaper ? { background: wallpaper } : undefined}>
      <header className="safe-top glass flex items-center gap-3 border-b border-line px-3 pb-3">
        ...
        <div className="grid h-10 w-10 place-items-center rounded-2xl bg-brand/20 font-bold text-brand">C</div>
        <p className="flex-1 font-semibold">Conversation</p>
      </header>
        ...
        {msgs.map((m) => {
          ...
          return (
            <div key={m.id} className={`flex ${mine ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[78%] rounded-2xl px-3 py-2 text-sm ${mine ? 'bg-brand text-white rounded-br-md' : 'bg-card border border-line/60 rounded-bl-md'}`}>
                {m.kind === 'text' && <p className="whitespace-pre-wrap break-words">{m.text}</p>}
                {(m.kind === 'gif' || m.kind === 'image') && <img src={m.mediaUrl} className="max-h-60 rounded-xl" />}
